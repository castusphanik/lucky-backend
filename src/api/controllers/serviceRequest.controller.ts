// src/api/controllers/serviceRequest.controller.ts
...
export async function createServiceRequest(req: Request, res: Response) {
  try {
    const {
      ...
      // optional DB fields
      service_type_lookup_id,
      saved_location_id,
      po_reference,
      driver_name,
      driver_phone,
      driver_with_unit, // Boolean in schema
    ...
    // ---- Requireds
    if (!urgency_code)
      return sendErrorResponse(res, "urgency_code is required", 400)
    if (!issue_description)
      return sendErrorResponse(res, "issue_description is required", 400)
    if (!created_at)
      return sendErrorResponse(
        res,
        "created_at (ISO datetime) is required",
        400
      )
    if (!created_by)
      return sendErrorResponse(res, "created_by is required", 400)
    ...
    if ((hasEquipmentId ? 1 : 0) + (hasExternalUnit ? 1 : 0) !== 1) {
      return sendErrorResponse(
        res,
        "Provide exactly one of equipment_id OR external_unit_number (not both).",
        400
      )
    }

    // ---- Resolve urgency_code
    const urgency = await prisma.service_urgency_lookup.findUnique({
      where: { urgency_code },
    ...
    if (!urgency)
      return sendErrorResponse(
        res,
        `Unknown urgency_code '${urgency_code}'`,
        400
      )
    ...
    if (anySecondary) {
      if (!secondary_contact_phone || !secondary_contact_email) {
        return sendErrorResponse(
          res,
          "secondary_contact_phone and secondary_contact_email are required when adding a secondary contact.",
          400
        )
      }
    }
    ...
    const data: any = {
      ...
      driver_with_unit:
        typeof driver_with_unit === "boolean" ? driver_with_unit : false,
    ...
    return sendSuccessResponse(res, responseBody, "Created", 201)
  } catch (err: any) {
    ...
    if (
      ...
      return sendErrorResponse(
        res,
        "attachment_id must be unique (already used)",
        400
      )
    }
    if (
      ...
      return sendErrorResponse(
        res,
        "Invalid attachment_id (FK constraint)",
        400
      )
    }

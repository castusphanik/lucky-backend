...
export const getMapView = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (Array.isArray(queryAccountId)) {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          if (!isNaN(singleId)) accountIds = [singleId]
        }
      } catch {
    ...
    const totalRecords = await prisma.equipment.count({
      where: {
      ...
      where: {
        equipment_assignment: {
          some: {
            equipment_type_allocation_ref: {
              account_id:
                accountIds.length === 1 ? accountIds[0] : { in: accountIds },
              //   is_deleted: false,
            },
          },
        },
    ...
    const filteredEquipments = equipments.map((equip: any) => ({
      ...equip,
      current_equipment_gps_location:
        equip.current_equipment_gps_location.filter(
          (gps: any) => gps.equipment_id === equip.equipment_id
        ),
    }))
...
export const getEquipmentDetails = async (req: Request, res: Response) => {
  try {
    ...
    const gps = await prisma.current_equipment_gps_location.findFirst({
    ...
    return res.status(200).json({
      ...
      gps: gps
        ? {
            ...
            motionStatus:
              gps.current_equipment_motion_status_lookup?.field_name ||
              gps.motion_status ||
              null,
            alarmCodeStatus:
              gps.current_equipment_alarm_status_lookup?.field_name ||
...
export const getListView = async (req: Request, res: Response) => {
  try {
    ...
    const {
      ...
      contractEndDate,
      agreement_type,
      equipment_load_status, // ✅ NEW FILTER
    } = req.query

    // Build equipmentRefFilter
    const equipmentRefFilter: any = {}
    ...
    // Fetch data
    const rawData = await prisma.equipment_assignment.findMany({
    ...
    if (equipment_id && String(equipment_id).trim() !== "") {
      ...
      filteredData = rawData.filter((item: any) =>
        item.equipment_ref.equipment_id.toString().includes(eqIdStr)
      )
    }
    ...
    // Format data
    const formatted = paginatedData.map((item: any) => {
      ...
      return {
        ...
        current_equipment_gps_location_id:
          gps?.current_equipment_gps_location_id || null,
        ...
        equipmentLoadStatus:
          loadDetail?.load_status_lookup?.field_name ||
    ...
    return res.status(200).json({
      ...
      data: JSON.parse(
        JSON.stringify(
          {
            success: true,
            tablename: column_preferences.fleet.fleet,
            totalRecords,
            currentPage: page,
            perPage,
            totalPages: Math.ceil(totalRecords / perPage),
...
export const getEquipmentCounts = async (req: Request, res: Response) => {
  let accountIds: number[] = []
  ...
  try {
    ...
    equipments.forEach((e) => {
      if (
        e.current_equipment_gps_location?.some(
          (g) => g.motion_status === "STOPPED"
        )
      ) {
    ...
    if (accountIds.length === 1) {
      geofenceCount = await prisma.geofence_alerts.count({
    ...
    if (filterBy === "gpsEquippedCount") {
      filteredData = rawData.filter((item) =>
        uniqueGpsEquipped.has(item.equipment_ref.equipment_id)
      )
    } else if (filterBy === "accessWithoutGpsCount") {
      filteredData = rawData.filter(
        (item) => !uniqueGpsEquipped.has(item.equipment_ref.equipment_id)
      )
    } else if (filterBy === "idleUnitsCount") {
      filteredData = rawData.filter((item) =>
        idleEquipIds.has(item.equipment_ref.equipment_id)
      )
      ...
      filteredData = rawData.filter((item) =>
        expiredInspectionIds.has(item.equipment_ref.equipment_id)
      )
...
export const downloadListView = async (req: Request, res: Response) => {
  try {
    ...
    await exporter.generateWorkbook({
      ...
      subtitle: createPaginationSubtitle(page, perPage, skip, totalRecords),
      columns,
      data: formatted,
      filename,
    })

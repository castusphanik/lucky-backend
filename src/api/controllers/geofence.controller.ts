...
export const createGeofenceCtrl = async (req: Request, res: Response) => {
  try {
    const {
      ...
      owner,
      tag_lookup_id,
      customer_id, // customer_id as org id
    ...
    if (
      ...
      return sendErrorResponse(
        res,
        "Missing required fields: geofence_name, shape_type, tag_lookup_id, customer_id, account_ids"
      );
    }
    ...
    if (shape_type === "Polygon" && !polygon) {
      return sendErrorResponse(
        res,
        "Polygon shape requires 'polygon' field with coordinates"
      );
    }
    ...
    if (
      ...
      return sendErrorResponse(
        res,
        "Circle shape requires center_lat, center_lng and radius_meters"
      );
    }

    // Insert into DB
    const [insertedRow]: { geofence_account_id: number }[] =
  ...
  `,
        ...
        radius_meters ? parseFloat(radius_meters) : null,
        owner,
        tag_lookup_id,
        description || null,
        geofence_location || null,
        status || "ACTIVE",
        created_by || owner,
        owner,
        event_master_id
    ...
    for (const equipment_id of equipmentIdArray) {
      ...
      let updatedGeofenceIds: number[] = [];

      if (equipmentRecord && equipmentRecord.geofence_account_id) {
        updatedGeofenceIds = equipmentRecord.geofence_account_id as number[];
...
export const fetchGeofencesByCustAndAccIdsCtrl = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    if (
      ...
      isNaN(parseInt(customer_id as string, 10))
    ) {
      return res.status(400).json({
        status: "error",
  ...
  `,
    ...
    const result = geofences.map((g: any) => {
      ...
      try {
        if (g.polygon) {
          ...
          polygon = geoJson.coordinates[0].map(
    ...
    return sendSuccessResponse(res, result, "Geofences fetched successfully");
  } catch (error) {
...
export const getGeofenceByIdCtrl = async (req: Request, res: Response) => {
  try {
    ...
    const geofence = await prisma.$queryRawUnsafe<any[]>(
      `
      SELECT 
...
`,
    ...
    // ------------------------
    return sendPaginatedResponse(res, result, total, page, perPage, 200);
  } catch (error) {
    console.error("Error fetching geofences by customer_id:", error);
    return sendErrorResponse(res, "Failed to fetch geofences");
  }
...
export const toggleGeofenceStatus = async (req: Request, res: Response) => {
  try {
    ...
    const geofence = await prisma.geofence_account.findUnique({
...
export const checkEquipmentInGeofenceCtrl = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    if (!accountId || isNaN(Number(accountId))) {
      return sendErrorResponse(res, "Valid account ID is required");
    }
  ...
  WHERE 
    ...
    if (!equipmentList || equipmentList.length === 0) {
      return sendSuccessResponse(
        res,
        [],
        "No equipment found for this account"
      );
    }
    ...
    const gpsLocations = await prisma.current_equipment_gps_location.findMany({
      where: {
        equipment_id: {
          in: equipmentIds,
        },
    ...
    for (const gps of gpsLocations) {
      ...
      for (const geofence of geofences) {
        ...
        const isInsideResult = await prisma.$queryRawUnsafe<any[]>(
          `
          SELECT ST_Contains(
    ...
    return sendSuccessResponse(
      res,
      entryEvents,
      "Checked equipment locations against geofences"
    );
  } catch (error) {
    console.error("Error in checkEquipmentInGeofenceCtrl:", error);
    return sendErrorResponse(res, "Failed to check equipment geofence status");
  }
};

export const checkPointInGeofenceCtrl = async (req: Request, res: Response) => {
  try {
    ...
    const result = await prisma.$queryRawUnsafe<{ is_inside: boolean }[]>(
      `
      SELECT ST_Contains(
        polygon,
    ...
    return sendErrorResponse(res, "Failed to check point in geofence");
  }
};

export const updateEquipmentLocationAndEventCreation = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    await prisma.current_equipment_gps_location.upsert({
      where: { equipment_id },
      update: {
        latitude: lat,
        longitude: lng,
    ...
    await prisma.equipment_gps_location.create({
      data: {
    ...
    const geofenceAccountIds =
      (equipment.geofence_account_id as number[]) ?? [];
...
`;
    ...
    `;
      ...
      const event_type: "entry" | "exit" = isInside ? "entry" : "exit";

      await prisma.equipment_geofence_event.create({
        data: {
          equipment_id,
    ...
    return sendErrorResponse(res, "Failed");
  }
};

export const checkLocationAndInsertGeofenceEvent = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const geofenceAccountIds: number[] =
      (equipment.geofence_account_id as number[]) ?? [];
    if (!geofenceAccountIds.length) {
      return res.json({
        message: "No geofences for this equipment",
...
`;
    ...
    for (const geofence of geofences) {
      if (!geofence.polygon_wkt) continue;
...
export const getGeofenceCountsCtrl = async (req: Request, res: Response) => {
  try {
    ...
    if (!custId || isNaN(customer_id)) {
      return sendErrorResponse(res, "Invalid or missing customer_id");
    }

    // ===============find the geofence counts===============//
    // Count total geofences for this customer_id (not deleted)
    const AllGeoFences = await prisma.geofence_account.count({
      where: {
        customer_id,
        is_deleted: false,
      },
    ...
    const geoFenceActive = await prisma.geofence_account.count({
      where: {
        customer_id,
        status: "ACTIVE",
        is_deleted: false,
    ...
    const geoFenceInActive = await prisma.geofence_account.count({
      where: {
        customer_id,
        status: {
          not: "ACTIVE",
    ...
    const accounts = await prisma.geofence_account.findMany({
      where: { customer_id },
    ...
    for (const account of accounts) {
      if (account.account_ids) {
        let parsed: unknown;
        try {
          parsed = JSON.parse(account.account_ids as any);
        } catch {
    ...
    return sendSuccessResponse(
      res,
      { geoFenceActive, geoFenceInActive, AllGeoFences },
      "Geofence counts retrieved successfully"
    );
    return sendSuccessResponse(res, "Geofence counts retrieved successfully");
  } catch (error) {
